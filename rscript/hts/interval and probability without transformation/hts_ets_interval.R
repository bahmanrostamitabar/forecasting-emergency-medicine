...
fmean <- fvar <- matrix(NA, nrow = fh, ncol = NCOL(ally))
for(i in seq(NCOL(ally))) {
  fc <- forecast(ets(ally[,i]), h = fh, level=95)
...
#Because we are using a transformation of the data (sqrt) before fitting the models, you need to compute ytilde, lower and upper on the transformed data. 
#Then square them all to give the results on the original scale.

ets_base <- fmean[,534:ncol(fmean)]
colnames(ets_base) <- colnames(incident_all_train)
fct_ets_bu <- hts::gts(ets_base , 
                           characters = list(c(1,2),3,9))
...
ets_comd_all <- (aggts(ytilde))
ets_bu_all <- (aggts(fct_ets_bu))
