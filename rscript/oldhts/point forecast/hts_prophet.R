...
date_train <- train %>% select(date) %>% rename(ds=date)
date_test <- test %>% select(date) %>% rename(ds=date)
...
incident <- holiday_dummy %>% bind_cols(incident_all) %>% select(-public_holiday_d,-school_holiday_d,-xmas,-new_years_day)
...
test_tscv <- incident %>% filter_index("2018-07-31" ~ .) %>%
  slide_tsibble(.size = f_horizon, .step = 1) %>% as_tibble()
