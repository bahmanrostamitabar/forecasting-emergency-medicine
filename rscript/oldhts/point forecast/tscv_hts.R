...
Container <- foreach(i = 1:2, .combine = rbind, .packages = "opera") %dopar% {
  ...
  all_approach <- tscount_c %>%
    bind_rows(ets_c) %>%
    bind_rows(arima_c) %>%
    bind_rows(tscount_bu) %>%
    bind_rows(ets_bu) %>%
    bind_rows(arima_bu)

  fcst_all <- all_approach %>%
    bind_cols(.id = rep(i, nrow(all_approach))) %>%
  ...
  test_set <- as_tibble(aggts(gts_incident_test)) %>%
    bind_cols(.id = rep(i, nrow(as_tibble(aggts(gts_incident_test))))) %>%
  ...
  error <- tscount_comd_e %>%
    bind_rows(tscount_bu_e) %>%
    bind_rows(arima_comb_e) %>%
    bind_rows(arima_bu_e) %>%
    bind_rows(ets_comb_e) %>%
    bind_rows(ets_bu_e)

  all_error <- error %>%
    bind_cols(.id = rep(i, nrow(error))) %>%
    select(.id, approach, everything())

  return(list(
    error_all_level = all_error,
