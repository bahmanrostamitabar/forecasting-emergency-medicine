...
tscount_f <- function(x,h) {
  xFourier <- forecast::fourier(msts(x,
                      ...
                      K=c(3,10))
  xreg_holiday_fourier <- cbind(xFourier,
...
prophetfit <- fit.prophet(
  ...
  df = data.frame(ds = date_train, 
                  y = sqrt(pull(x))
                  )
)

prophetfitted <- round(predict(prophetfit, 
                         data.frame(ds = date_test))$yhat)
prophetfitted
}
...
y.f <- combinef(allf, groups = get_groups(y), 
