...
**Response:** Thank you for your comment. We agree that it would be helpful to readers to have more information about the specific input predictors used in the GLM and TSGLM models. To address this, we have revised the manuscript to include the model formulas in R-format, as you suggested.
...
**Response:** Thank you for your follow-up comment regarding the Poisson assumption used in our analysis. Regarding the potential issues of over-dispersion and under-dispersion, we acknowledge in the manuscript that these can be concerns when using Poisson regression models and included references to relevant literature that discuss these issues in more detail. We use the Poisson distribution to model the count data in our analysis. This is because the Poisson distribution is appropriate for count data that are non-negative and discrete, and it produces strictly positive forecasts. This is particularly important for some of the time series in our dataset, especially the bottom-level series, which have low volume and are more susceptible to producing negative forecasts if other distributions were used. Additionally, we have developed a flexible framework that can incorporate any forecasting model, but it is important to note that the intention of this paper is not to provide a comprehensive list of models. Instead, our focus is on demonstrating the usefulness of our framework for incorporating base forecasts generated by any model and producing hierarchical coherent forecasts.
...
Furthermore, we did consider other models specifically designed for integer-valued time series, such as TSGLM and GLM. We acknowledge that there are other methods available, such as INAR or INGARCH models. However, the intention of this study was not to provide a comprehensive list of forecasting models, but rather to develop a framework that can incorporate any forecasting model.

_Comment 6 (Check with Rob): How to understand the stationarity of the time series? Is it going to impact the analysis if the errors are assumed additive and the heteroscedasticity is present?_

**Response:** Thank you for your question. 

Thank you for your comment. Stationarity refers to the property of a time series where its statistical properties (such as mean, variance) remain constant over time. In practical terms, a stationary time series has consistent behavior over time, regardless of when it is observed. To assess the stationarity of a time series we could extrcat features based on a test such as the Kwiatkowski-Phillips-Schmidt-Shin (KPSS) test. These tests check whether a time series has a unit root, which is a stochastic trend that causes non-stationarity. Some models, such as ETS models, are designed to work with non-stationary time series. GLM and TSGLM can handle non-stationary time series by allowing for time-varying parameters that can capture changes in the underlying chnages of the data over time. They also allow for the inclusion of external regressors in the model, which can further help to capture different patterns in the data and be adapted to a wide range of time series , including non-stationary data.

Regarding your question about the impact of additive errors and heteroscedasticity on the analysis, in general, assuming additive errors does not require stationarity of the time series. However, if the time series is not stationary, the estimation of the error terms may be biased. Heteroscedasticity, which refers to the changing variance of the error terms over time, may also affect the estimation of the model parameters and the forecast accuracy that can be addressed by using a Box-Cox transformation.
...
**Response:**  Thank you for the question A possible adjustment could be applying the Box-Cox transformation before the forecasting. Also, alternative models such as the Negative Binomial Regression may be more appropriate. Both GLM and TSGLM allow for the use of  Negative Binomial, if required.
...
**Response:** Thank you. The aggregation matrix A is determined by the structure of the hierarchy. It maps the bottom-level time series to the corresponding higher-level time series, and recursively aggregates the time series at each level up to the top-level time series. The matrix A is typically predefined based on the structure of the hierarchy, and it remains fixed throughout the forecasting process. (Maybe we can show an example)



_– (check with Rob) for the linear reconciliation method $\tilde{y}_h = S(S'W^{-1}S)^{-1}W^{-1}\hat{y}_h,$ how is it derived?_
...
**Response:** Thank you or your comment. 

Ordinary Least Squares (OLS) is the simplest and most commonly used method for estimating the parameters in linear regression models. In this approach, the estimation of W is based on the assumption that all the errors have equal variance. Hence, W is simply defined as the identity matrix multiplied by a constant factor. The intuitions behind OLS is that it minimizes the sum of squared residuals between the observed and predicted values of the dependent variable, assuming the errors are normally distributed.

Weighted Least Squares (WLS): WLS is an extension of OLS where the variance of the errors is assumed to be heteroscedastic, i.e., different for each observation. In this approach, W is defined as a diagonal matrix with the reciprocal of the variance of the errors on the diagonal. The intuitions behind WLS is that it assigns higher weight to observations with smaller variance of the errors, resulting in a better fit for the entire data set.

Minimum Trace (MinT): MinT is a method that aims to minimize the trace of W, subject to the constraint that W is a positive definite matrix. It is often used in cases where the number of variables is larger than the number of observations, resulting in an underdetermined system of equations. In this approach, W is defined as the inverse of the covariance matrix of the errors. The intuitions behind MinT is that it shrinks the estimates towards zero and produces more stable results, particularly when the data has multicollinearity.
...
**Response:** Thank you for providing minor comment. We have corrected the text accordingly.
...
**Response:** Thank you for your comment. We have removed the indentation after equations in paragraphs that do not end.


_- P14, L3, for the Continuous Rank Probability Score, how is the true probability distribution function F_j (x) determined?_

**Response:** Thank you. CRPS measures the difference between the forecasted cumulative distribution function (CDF) and the actual CDF of the forecast variable. The CRPS is computed as the integral of the squared difference between the forecasted CDF and the actual CDF, over the range of possible values. To provide a more specific example, suppose we have a time series of ambulance demand, and we want to forecast the demand for the next day The true probability in each day is the probability of observing the actual demand, which is always 1. The CRPS measures the difference between the forecasted probability distribution of the demand and the actual probability distribution of the demand. The yellow area in the plot of the forecasted CDF and the actual CDF highlights this difference.
...
**Response:** Thank you for your suggestion.Throughout, we ensured that a consistent format was maintained in the References section regarding the inclusion or exclusion of URL links.
...
**Response:** Thank you. We have reviewed Table 2 revised the table to make the column names more clear and consistent with Table 1.
