...
f_horizon <- 5*7
# Read data
incidents_original <- readxl::read_excel("Nature_of_Incidents_Attended.xlsx") %>%
...
# read holiday data
holiday <- readxl::read_excel("holiday_rugby.xlsx") %>% 
...
train_tscv <- incidents_gts_holidays %>% filter_index(. ~ "2019-06-27") %>% 
