...
error_m <- error %>%  select(-approach,-.id) %>% as.matrix()
...
ets_hts_rmsse_long_h <- ets_hts_rmsse_long %>% select(level,.id,approach,error2_scale) %>% 
group_by(level,.id,approach) %>% mutate(h= row_number())
...
ets_hts_rmsse_long_h <- ets_hts_rmsse_long %>% select(level,.id,approach,error_scale) %>% 
  group_by(level,.id,approach) %>% mutate(h= row_number())
...
rmss_hts <- ets_hts_rmsse_long %>% group_by(approach,.id,level)  %>% summarise(rmsse=sqrt(mean(error2_scale)))

rm <- rmss_hts %>% filter(!is.nan(rmsse))

rm %>% group_by(approach) %>% summarise(rmsse_mean=mean(rmsse),rmsse_median=median(rmsse))
